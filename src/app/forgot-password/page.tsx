...
export default function ForgotPasswordPage() {
  ...
  return <main className="alfredPage grid min-h-dvh place-items-center px-5 py-10"><Card className="w-full max-w-md"><BrandMark className="mb-6" /><h1 className="font-brand text-4xl font-semibold">{labels.forgotTitle}</h1><p className="mt-2 text-sm text-[var(--text-secondary)]">{labels.forgotSubtitle}</p><form onSubmit={onSubmit} className="mt-6 grid gap-4"><FieldLabel label={labels.email}><Input name="email" type="email" autoComplete="email" required /></FieldLabel>{message ? <p role="status" className="text-sm font-semibold text-emerald-500">{message}</p> : null}{error ? <p role="alert" className="text-sm font-semibold text-red-500">{error}</p> : null}<Button type="submit" disabled={loading || Boolean(message)}>{loading ? labels.loading : labels.sendLink}</Button><Button href="/login" variant="secondary">{labels.backToLogin}</Button></form></Card></main>;
}
