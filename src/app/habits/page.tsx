...
export default function HabitsPage() {
  ...
  return (
    <AppShell title={labels.title} infoPage="habits">
      <div className="flex items-center justify-between gap-4"><div data-tour="habits-title"><SectionTitle title={labels.heading} description={labels.description} /></div><span data-tour="habit-add"><Button href="/goals">{labels.add}</Button></span></div>
      <Card data-tour="habit-guide" className="habitGuideCard grid gap-3"><p className="habitGuideText text-sm font-semibold leading-6">{labels.consistencyGuide}</p><div className="flex flex-wrap gap-2 text-xs font-bold text-[var(--text-tertiary)]"><span data-tour="habit-consistency-fire" className="habitGuidePill habitGuidePillFire">{labels.fireGuide}</span><span data-tour="habit-consistency-grass" className="habitGuidePill habitGuidePillGrass">{labels.grassGuide}</span><span data-tour="habit-consistency-ice" className="habitGuidePill habitGuidePillIce">{labels.iceGuide}</span><span data-tour="habit-consistency-empty" className="habitGuidePill habitGuidePillEmpty">{labels.emptyGuide}</span></div><p className="text-xs text-[var(--text-tertiary)]">{habits.data?.length ?? items.length} hábitos ativos · semana de {range.start} a {range.end}</p></Card>
