...
export default function HabitsPage() {
  ...
  return (
    <AppShell title={labels.title}>
      ...
      <Card className="grid gap-3"><p className="text-sm font-semibold leading-6">{labels.consistencyGuide}</p><p className="text-xs text-[var(--text-tertiary)]">{habits.data?.length ?? items.length} hábitos ativos · semana de {range.start} a {range.end}</p></Card>
      ...
      {items.map((item) => <HabitCard key={item.habit.id} item={item} onLog={logHabit} onEdit={openEdit} onDelete={(id) => { if (window.confirm("Excluir este hábito permanentemente?")) mutation.mutate(() => routineApi.deleteHabit(id)); }} />)}
      {!dashboard.isLoading && !items.length ? <EmptyState title={labels.emptyTitle} description={labels.emptyDescription} href="/goals" /> : null}
      {editing ? <div className="fixed inset-0 z-50 grid place-items-end bg-black/50 p-4 sm:place-items-center"><Card className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto"><form onSubmit={saveEdit} className="grid gap-3"><h2 className="text-xl font-bold">Editar hábito</h2><FieldLabel label={labels.habitName}><Input name="name" defaultValue={editing.name} minLength={2} maxLength={60} required /></FieldLabel><FieldLabel label="Meta"><Select name="goalId" defaultValue={editing.goal_id ?? ""}>{goals.data?.map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</Select></FieldLabel><FieldLabel label="Duração (min)"><Input name="duration" type="number" min={1} max={1440} defaultValue={editing.duration_minutes} required /></FieldLabel><FieldLabel label="Recorrência"><Select value={editRecurrence} onChange={(event) => setEditRecurrence(event.target.value as RecurrenceFrequency)}><option value="daily">Diário</option><option value="weekly">Semanal</option><option value="monthly">Mensal</option><option value="yearly">Anual</option></Select></FieldLabel>{editRecurrence === "weekly" ? <DayPicker name="weekDays" count={7} defaults={parseRRule(editing.recurrence_rule).frequency === "weekly" ? parseRRule(editing.recurrence_rule).selected : [1,2,3,4,5]} /> : editRecurrence === "monthly" ? <DayPicker name="monthDays" count={31} defaults={parseRRule(editing.recurrence_rule).frequency === "monthly" ? parseRRule(editing.recurrence_rule).selected : [1]} /> : null}<FieldLabel label={labels.reason}><Textarea name="description" maxLength={200} defaultValue={editing.description ?? ""} /></FieldLabel><div className="grid grid-cols-2 gap-2"><Button type="button" variant="secondary" onClick={() => setEditing(null)}>Cancelar</Button><Button type="submit" disabled={mutation.isPending}>Salvar</Button></div></form></Card></div> : null}
    </AppShell>
  );
}
