...
export default function InsightsPage() {
  ...
  return <AppShell title={labels.title} infoPage="insights"><Card data-tour="insights-summary" className="grid gap-4"><p className="text-sm text-[var(--text-secondary)]">{labels.completion}</p><h2 className="text-5xl font-black">{completion}%</h2><ProgressBar value={completion} /></Card><SectionTitle title={labels.patterns} description={labels.patternsDescription} /><div data-tour="insights-patterns" className="grid gap-4">{goalItems.map((item)=><Card key={item.goal.id}><div className="flex justify-between gap-4"><div><h3 className="text-lg font-bold">{item.goal.title}</h3><p className="mt-2 text-sm text-[var(--text-secondary)]">{item.completed_count} concluídos · {item.uncompleted_count} não concluídos · {item.pending_count} pendentes</p></div><Badge tone={item.consistency_level==="fire"?"green":item.consistency_level==="grass"?"blue":item.consistency_level==="ice"?"amber":"neutral"}>{Math.round(item.consistency_percent)}%</Badge></div></Card>)}{!goals.isLoading&&!goalItems.length?<EmptyState title={labels.emptyTitle} description={labels.emptyDescription} href="/goals" />:null}</div></AppShell>;
}
