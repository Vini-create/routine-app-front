...
export default function DashboardPage() {
  ...
  const entries = agendaEntries(agenda.data, preferredTimes);
  ...
  return <AppShell title={labels.title} showTitle={false} mainClassName="lg:gap-10">
    ...
    <section className="grid gap-5"><SectionTitle title={labels.habits} description={labels.overviewText} />{habits.data?.habits.length ? <div className="grid gap-4 lg:grid-cols-2">{habits.data.habits.slice(0,2).map((item) => <HabitCard key={item.habit.id} item={item} preferredTime={preferredTimes[item.habit.id]} />)}</div> : <EmptyState title={labels.emptyHabitsTitle} description={labels.emptyHabitsDescription} href="/habits" />}</section>
  </AppShell>;
}
