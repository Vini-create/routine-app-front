    ...
    <AppShell title={labels.title}>
      ...
      <Card className="grid gap-4">
        ...
        <form onSubmit={saveDefaultRoutineItem} className="grid gap-3">
          <div className="grid grid-cols-2 gap-3"><FieldLabel label={labels.titleField}><Input name="defaultTitle" placeholder={labels.defaultRoutinePlaceholder} minLength={2} maxLength={150} required /></FieldLabel><FieldLabel label={labels.time}><TimeSelect name="defaultTime" defaultValue="08:00" required /></FieldLabel></div>
          <div className="grid grid-cols-2 gap-3"><FieldLabel label={labels.duration}><Input name="defaultDuration" type="number" min={1} max={1440} defaultValue={60} required /></FieldLabel><FieldLabel label={labels.recurrence}><Select value={defaultRecurrence} onChange={(event) => setDefaultRecurrence(event.target.value as RecurrenceFrequency)}><option value="weekly">{labels.weekly}</option><option value="monthly">{labels.monthly}</option></Select></FieldLabel></div>
