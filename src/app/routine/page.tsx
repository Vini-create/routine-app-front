  ...
  return (
    <AppShell title={labels.title} infoPage="routine">
      ...
      {view === "week" ? Array.from({ length: 7 }, (_, index) => addDays(fromDateKey(range.start), index)).map((date) => {
        ...
        return <Card key={key}><div className="flex min-w-0 items-center justify-between gap-3"><Badge className="min-w-0 whitespace-normal break-words" tone={key === today ? "green" : "blue"}>{date.toLocaleDateString(language, { weekday: "long", day: "2-digit", month: "2-digit" })}</Badge><span className="shrink-0 text-xs font-bold text-[var(--text-tertiary)]">{dayEntries.length} itens</span></div><div className="mt-4 grid min-w-0 gap-2">{dayEntries.map((entry) => <div key={entry.key} className="flex min-w-0 justify-between gap-3 rounded-2xl bg-[var(--surface-ambient)] px-4 py-3"><strong className="min-w-0 break-words [overflow-wrap:anywhere]">{entry.time} · {entry.title}</strong><span className="shrink-0 text-xs">{{ completed: statusLabels.done, uncompleted: statusLabels.missed, pending: statusLabels.pending, vacation: statusLabels.vacation }[entry.status]}</span></div>)}</div></Card>;
      }) : entries.map((entry) => {
      ...
      <Card className="grid gap-4">
        ...
        <form onSubmit={saveDefaultRoutineItem} className="grid gap-3">
          <div className="grid gap-3 sm:grid-cols-2"><FieldLabel label={labels.titleField}><Input name="defaultTitle" placeholder={labels.defaultRoutinePlaceholder} minLength={2} maxLength={150} required /></FieldLabel><FieldLabel label={labels.time}><TimeInput name="defaultTime" defaultValue="08:00" required /></FieldLabel></div>
          <div className="grid gap-3 sm:grid-cols-2"><DurationInput name="defaultDuration" label={labels.duration} defaultMinutes={60} /><FieldLabel label={labels.recurrence}><Select value={defaultRecurrence} onChange={(event) => setDefaultRecurrence(event.target.value as RecurrenceFrequency)}><option value="weekly">{labels.weekly}</option><option value="monthly">{labels.monthly}</option></Select></FieldLabel></div>
      ...
      <Card className="grid gap-4"><SectionTitle title={labels.vacationPeriod} description={labels.vacationDescription} />
        <form onSubmit={applyVacation} className="grid min-w-0 gap-4"><div className="grid min-w-0 gap-3 min-[360px]:grid-cols-2"><FieldLabel label={labels.startDate}><Input name="vacationStart" type="date" min={today} required /></FieldLabel><FieldLabel label={labels.endDate}><Input name="vacationEnd" type="date" min={today} required /></FieldLabel></div><div className="grid min-w-0 gap-2 sm:grid-cols-2">{recurringItems.map((item) => <label key={item.id} className="flex min-w-0 items-center gap-3 overflow-hidden rounded-2xl border border-[var(--border-soft)] p-4"><input type="checkbox" name="itemIds" value={item.id} className="size-5 shrink-0" /><span className="min-w-0"><strong className="block break-words [overflow-wrap:anywhere]">{item.title}</strong><span className="block break-words text-xs text-[var(--text-tertiary)] [overflow-wrap:anywhere]">{describeRRule(item.recurrence_rule ?? "", language)}</span></span></label>)}</div>{recurringItems.length ? <Button type="submit" variant="secondary" disabled={mutation.isPending}>{labels.saveVacation}</Button> : <p className="text-sm text-[var(--text-secondary)]">{labels.noVacationItems}</p>}</form>
      </Card>
...
function DayPicker({ name, count, defaults }: { name: string; count: number; defaults: number[] }) {
  return <div className="grid min-w-0 grid-cols-7 gap-1 sm:gap-1.5">{Array.from({ length: count }, (_, index) => count === 7 ? index : index + 1).map((day) => <label key={day} className="grid min-h-10 min-w-0 place-items-center rounded-2xl bg-[var(--surface-ambient)] text-xs font-bold"><input className="peer sr-only" type="checkbox" name={name} value={day} defaultChecked={defaults.includes(day)} /><span className="grid size-full min-w-0 place-items-center rounded-2xl peer-checked:bg-[var(--text-primary)] peer-checked:text-[var(--background-primary)]">{count === 7 ? ["D","S","T","Q","Q","S","S"][day] : day}</span></label>)}</div>;
}
